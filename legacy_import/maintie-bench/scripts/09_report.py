...
def _docs(recs):
    # Use the ORIGINAL gold entities (MaintIE has ~30% nested/overlapping spans
    # that BIO cannot represent); fall back to BIO-derived only if absent.
    return [{"tokens": d["tokens"], "entities": d.get("entities") or bio_to_entities(d["tokens"], d["bio"]),
...
def main():
    ...
    json.dump({"split": split_note, "n_train": len(tr), "n_test": len(te),
               "metrics": metrics}, open("outputs/reports/metrics_full.json", "w"), indent=2, default=str)
    ...
    print(f"  {'MODEL':<24}{'ENTITY F1':>12}{'RELATION F1':>14}")
    print(f"  {'-'*24}{'-'*12:>12}{'-'*14:>14}")
    for n, m in metrics.items():
        print(f"  {n:<24}{m['entity']['micro_f1']:>12.4f}{m['relation']['micro_f1']:>14.4f}")
    print(f"\nSaved tables -> {TABDIR}/  figures -> {FIGDIR}/  + metrics_full.json")

...
def main():
    ...
    ap.add_argument("--seeds", type=int, default=1, help=">1 = multi-seed mean±std for neural tiers")
    ...
    if args.seeds > 1:
        ...
        for t in args.tiers:
            ...
            for s in range(args.seeds):
                p = _tier2(tr, dv, te, schema, 42 + s) if t == "2" else _tier3(tr, dv, te, schema, 42 + s)
                ent.append(entity_scores(te_docs, p)["micro_f1"])
                rel.append(relation_scores(te_docs, p)["micro_f1"])
                print(f"  {NAMES[t]} seed {42+s}: entity={ent[-1]:.4f} relation={rel[-1]:.4f}")
            vrows.append({"model": NAMES[t], "seeds": args.seeds,

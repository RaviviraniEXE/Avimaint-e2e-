...
class BiLSTMSpanNER:
    ...
    def fit(self,records,dev=None,epochs=30,lr=1e-3,batch_size=32,patience=6):
        ...
        total=(len(train)+batch_size-1)//batch_size
        print(f"[BiLSTM Span-NER] device={self.device} | candidates={len(train)} | batches/epoch={total}")
        for epoch in range(1,epochs+1):
            rng.shuffle(train);self.net.train();running=0.0
            bar=tqdm(range(0,len(train),batch_size),total=total,desc=f"BiLSTM SPAN epoch {epoch:02d}/{epochs}",unit="batch",dynamic_ncols=True,leave=True)
            for step,i in enumerate(bar,1):
            ...
            with torch.no_grad():
                val_bar=tqdm(range(0,len(check),batch_size),total=(len(check)+batch_size-1)//batch_size,desc=f"BiLSTM SPAN dev   {epoch:02d}/{epochs}",unit="batch",dynamic_ncols=True,leave=False)
                for i in val_bar:
            ...
            print(f"[BiLSTM Span-NER] epoch={epoch:02d} dev_loss={current:.6f} best={best:.6f} patience={stale}/{patience}")
            if stale>=patience:
                print(f"[BiLSTM Span-NER] early stopping at epoch {epoch}; restoring best checkpoint")
                break
    ...
    def predict(self,records,batch_size=64):
        ...
        with torch.no_grad():
            for row in tqdm(records,desc="BiLSTM SPAN predict",unit="doc",dynamic_ncols=True,leave=True):
...
class TransformerSpanNER:
    ...
    def fit(self,records,dev=None,epochs=30,lr=3e-5,batch_size=8,patience=4):
        ...
        best,best_state,stale=float("inf"),None,0;rng=random.Random(self.seed)
        total=(len(train)+batch_size-1)//batch_size
        print(f"[Transformer Span-NER] device={self.device} | candidates={len(train)} | batches/epoch={total}")
        for epoch in range(1,epochs+1):
            rng.shuffle(train);self.encoder.train();self.head.train();running=0.0
            bar=tqdm(range(0,len(train),batch_size),total=total,desc=f"Transformer SPAN epoch {epoch:02d}/{epochs}",unit="batch",dynamic_ncols=True,leave=True)
            for step,i in enumerate(bar,1):
            ...
            with torch.no_grad():
                val_bar=tqdm(range(0,len(check),batch_size),total=(len(check)+batch_size-1)//batch_size,desc=f"Transformer SPAN dev   {epoch:02d}/{epochs}",unit="batch",dynamic_ncols=True,leave=False)
                for i in val_bar:
            ...
            print(f"[Transformer Span-NER] epoch={epoch:02d} dev_loss={current:.6f} best={best:.6f} patience={stale}/{patience}")
            if stale>=patience:
                print(f"[Transformer Span-NER] early stopping at epoch {epoch}; restoring best checkpoint")
                break
    ...
    def predict(self,records,batch_size=16):
        ...
        from tqdm.auto import tqdm
        with torch.no_grad():
            for row in tqdm(records,desc="Transformer SPAN predict",unit="doc",dynamic_ncols=True,leave=True):
